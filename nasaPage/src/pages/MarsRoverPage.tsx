    ...
    return (
        <div className='mars-page'>
            ...
            <nav>
                ...
                <select id="rover-dropdown" className='hidden' onChange={(e) => setSelectedRover(e.target.value)}>
                    <option value="Curiosity">Curiosity</option>
                    <option value="Opportunity">Opportunity</option>
                    <option value="Spirit">Spirit</option>
                </select>
                <select id="camera-dropdown" className='hidden' onChange={(e) => setSelectedCamera(e.target.value)}>
            ...
            <div className="mars-page-content">
                <div className='mars-rover-list'>
                    ...
                    {!isLoading && filteredPhotos.length === 0 && <h1 className='no-photos'>No Photos Found</h1>}
                    {!isLoading && filteredPhotos.map((data) => (
                        <MarsRover key={data.id} id={data.id} image={data.img_src} />
                    ))}
                </div>
            </div>
            <footer>
